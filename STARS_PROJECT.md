...
> **项目**: `stars-project/` · **版本**: v7.1 · **最后更新**: 2026-06-27
>
...
| **v7.1** | **2026-06-27** | **太阳系可见性修复**（5项）：场景雾密度 0.00015→0.00003（让远处行星可见）、随机行星远离太阳系（respawnMin 600→3000, respawnMax 1800→5000）、行星 emissive 发光（强度 0.2，黑暗背景中更显眼）、卫星 emissive 发光、引擎调试日志；**粒子穿越感大幅增强**：三层视差架构（背景慢速×0.3 + 中间中速×1.0 + 前景快速×2.5）、粒子数 3000→5000、分布范围 40→80、GPU 流动强度 8→20、CPU 流动强度 ×2→×6、粒子尺寸 200→350、静止时基础透明度 0.15、冲刺时透明度×1.5、三层不同色调（暖白/蓝白/亮蓝）；**银河系优化**：粒子密度 5000→8000、旋臂更紧凑（armSpread 0.45→0.3、spin 1.5→2.0）、高斯随机分布（参考 GalaxyThreeJS）、银河雾气层（1000 大粒子模拟尘雾）；**速度线增强**：数量 300→500、透明度上限 0.9→1.0、线段更长更亮 |
