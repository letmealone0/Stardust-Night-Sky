...
#### 公转时间为何不采用真实时间（v-latest 调整）

轨道**形状**完全写实——半长轴 `a`、偏心率 `e`、倾角 `i`、近日点幅角 `ω` 均取自真实数据，
开普勒方程（Newton-Raphson 求解）也真实还原了椭圆运动与开普勒第二定律（近日点加速、远日点变慢）。
但公转**周期**没有使用真实时间，原因如下：

1. **真实周期相差近 760 倍**：恩克约 3.3 年、哈雷约 75.9 年、斯威夫特-塔特尔约 133 年、
   海尔-波普约 2500 年。若按真实周期驱动（即使与行星同用 `TIME_SCALE`），海尔-波普一圈需
   数小时，肉眼几乎不动，用户会感觉"看半天都不动"。
2. **彗星与行星尺度不可比，相对周期无观感意义**：彗星轨道在 200~16000 AU，行星在 0.4~30 AU，
   两者差几百倍。用户在同一画面里无法同时看清彗星与行星，因此"彗星周期是否和行星一致"对
   体验毫无影响；而"彗星自己动不动得起来"才重要。
3. **即便与行星统一时间倍率，问题反而更严重**：原太阳系 `TIME_SCALE=30`（1 秒≈30 天），若把
   彗星也改成 30，海尔-波普一圈会变成约 8 小时——比原来的 2.8 小时更糟。

**解决方案（UX 优先）**：仅对公转周期做指数压缩（`orbitPeriodCompress = 1/3`），在保留"真实快慢
顺序"（恩克最快 → 海尔-波普最慢）的前提下，把最慢彗星压到 `orbitBaseSeconds`（默认 90 秒）可见。
压缩后各彗星视觉周期约为：恩克 10s、哈雷 28s、斯威夫特-塔特尔 34s、海尔-波普 90s。轨道形状与
运动规律仍科学准确，开普勒第二定律带来的近日点"甩尾加速"也保留。
...
```javascript
{
  ...
  comets: {                            // 彗星系统（v-latest: 公转速度可调）
    count: 4,                          // 标志性周期彗星数量
    orbitTimeScale: 1.0,               // 全局公转速度倍率（>1 更快，<1 更慢）
...
| 彗星公转周期不采用真实时间 | 真实周期差 ~760 倍，用真实时间最慢彗星一圈需数小时（几乎不动）；彗星与行星尺度不可比，相对周期无观感意义。故仅压缩周期（指数 1/3）保留真实快慢顺序，轨道形状(a/e/i/ω)与开普勒运动仍写实 |
...
| **Comets** | v3.0 | 4 颗标志性彗星（哈雷/海尔-波普/恩克/斯威夫特），开普勒轨道，十字双尾，流动动画，LOD；v-latest 公转周期指数压缩(1/3)保留真实快慢顺序，不用真实时间 |
